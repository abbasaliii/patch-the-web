...
test("a nontechnical user gets a complete installation and first-repair path", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("link", { name: /Download v0.10.0/ })).toHaveAttribute("href", "/downloads/patch-the-web-extension-v0.10.0.zip");

  const downloadPromise = page.waitForEvent("download");
  await page.getByRole("link", { name: /Download v0.10.0/ }).click();
  expect((await downloadPromise).suggestedFilename()).toBe("patch-the-web-extension-v0.10.0.zip");
